...
def suffix_array_to_suffix_tree(sa, lcp, text):
    ...
    for i in range(len(text)):
       suffix = sa[i]
       while string_depth_[curr] > lcp_prev:
          curr = parent.get(curr)
          print('sss')
       if string_depth_[curr] == lcp_prev:
          #make leaf
          parent[curr + 1] = curr
          tree[curr][mapper[text[suffix + string_depth_[curr]]]]=[curr + 1, suffix + string_depth_[curr], len(text) - 1]
          tree[curr + 1] = NA.copy()
          string_depth_[curr + 1] = len(text) - i
          curr = curr + 1
       else:
          edge_start = sa[i - 1] + string_depth_[curr]
          offest = lcp_prev - string_depth_[curr]
          #make edge
          start_char = text[edge_start]
          mid_char = text[edge_start + offest]
          parent[curr + 1] = curr
          tree[curr][mapper[start_char]] = [curr + 1, edge_start, edge_start + offest - 1]
          tree[curr + 1] = NA.copy()
          tree[curr + 1][mapper[mid_char]] = tree[curr][mapper[start_char]][0]
          parent[tree[curr][mapper[start_char]][0]] = curr + 1
          tree[curr][mapper[start_char]][1] += offest
          #create leaf
          parent[curr + 2] = curr + 1
          tree[curr + 1][mapper[text[suffix + string_depth_[curr]]]]=[curr + 1, suffix + string_depth_[curr], len(text) - 1]
          string_depth_[curr] = len(text) - i
          curr = curr + 2
       if i < len(text) - 1:
          lcp_prev = lcp[i]
    return tree
